...
public class CitaDAO {
    ...
    public static int addCita(Cita cita){
        int numFilas=0;
        String sql = "INSERT INTO CITA(fecha_hora, observaciones, tratamiento, trabajadorEncargado, clientePedido) VALUES (STR_TO_DATE(?,'%d-%m-%Y'), ?, ?, ?, ?)";
        try(Connection con = DataBaseManager.getConnection()){
    ...
    public static int actualizarCita(Cita cita){
        int numFilas = 0;
             ...
             */
        String sql = "UPDATE CITA SET fecha_hora = STR_TO_DATE(?,'%d-%m-%Y'), observaciones = ?, tratamiento = ?, trabajadorEncargado = ?, clientePedido = ? WHERE cod_cita = ?";
        try(Connection con = DataBaseManager.getConnection()){
            PreparedStatement sentencia = con.prepareStatement(sql);
            sentencia.setString(1, String.valueOf(cita.getFecha_hora()));
            sentencia.setString(2, cita.getObservaciones());
    ...
    public static Cita buscarCita(int cod_cita){
        ...
        try(Connection con = DataBaseManager.getConnection()){
            ...
            while (consulta.next()) {
                ...
                Trabajador trabajador = TrabajadorDAO.buscarTrabajador(cod_trabajador);
                Tratamiento tratamiento = TratamientoDAO.buscarTratamiento(cod_trat);
                LocalDate fecha_horaf = LocalDate.parse(fecha_hora);
                cita= new Cita(cod_cita, fecha_horaf, observaciones, trabajador, tratamiento, cliente);
            }
        }catch (SQLException e){}
        return cita;
    }
    ...
    public static boolean otherFechaHora(Cita cita) {
        ...
        try(Connection con = DataBaseManager.getConnection()){
            String sql = "SELECT * FROM CITA WHERE ID != ? AND FECHA_HORA = STR_TO_DATE(?,'%d-%m-%Y') AND trabajadorEncargado = ?";
            PreparedStatement sentencia = con.prepareStatement(sql);
